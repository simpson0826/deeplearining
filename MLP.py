...
# 重新產生資料產生器
DOE_result = pd.DataFrame(pd.read_csv('/home/asis/PM2.5_GPU/DOE/DOE_result_MLP.csv'))
time=[]
Training_loss=[]
Training_mae=[]
validation_loss=[]
...
for i in DOE_result.index:
    ...
    model.add(layers.Dense(unit1, activation=act1))
    model.add(layers.Dropout(drop1))
    model.add(layers.Dense(unit2, activation=act2))
