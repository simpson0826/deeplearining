...
# 重新產生資料產生器
DOE_result = pd.DataFrame(pd.read_csv('/home/asis/PM2.5_GPU/DOE/DOE_result_concant1.csv'))
time=[]
Training_loss=[]
Training_mae=[]
validation_loss=[]
...
for i in DOE_result.index:
    ...
    model = Sequential()
    model.add(layers.Conv1D(unit1,filterSize,
                         input_shape=(None, float_data.shape[-1]),
                         activation=act1))
    model.add(layers.MaxPooling1D(poolSize))
