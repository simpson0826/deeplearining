...
from keras.layers import Dropout, Activation, Flatten, GRU,Concatenate,Conv1D,MaxPooling1D,GlobalMaxPooling1D
#讀入檔案
# 設定路徑
...
# 重新產生資料產生器
DOE_result = pd.DataFrame(pd.read_csv('/home/asis/PM2.5_GPU/DOE/DOE_result_concant2.csv'))
time=[]
Training_loss=[]
Training_mae=[]
validation_loss=[]
...
for i in DOE_result.index:
    ...
    cnn_x = Input(shape=(None, float_data.shape[-1]))
    cnn_conv1 = Conv1D(unit1,filterSize, activation=act1)(cnn_x)
    cnn_pooling1 = MaxPooling1D(poolSize)(cnn_conv1)
    cnn_output = Dense(1)(cnn_pooling1)
    # GRU model # input layer
    gru_x = Input(shape=(None, float_data.shape[-1]))
    gru_hidden1 = GRU(unit2,
                      dropout=drop2,
                      recurrent_dropout=dropRnn2,
                      activation=act2)(gru_x)
    gru_output = Dense(1)(gru_hidden1)
    # merge feature
    merge = Concatenate()([cnn_output, gru_output])
    merge_hidden1 = Dense(unit3, activation=act3)(merge)
    merge_hidden2 = Dropout(drop3)(merge_hidden1)
    merge_output = Dense(1)(merge_hidden2)
    model= Model([cnn_x,gru_x],[merge_output])
    model.summary()
    # 開始訓練
